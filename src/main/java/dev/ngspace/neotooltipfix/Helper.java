package dev.ngspace.neotooltipfix;

import java.util.List;

import dev.ngspace.neotooltipfix.mixin.OrderedTextToolTipAccessor;
	...
	private static void wrapLongLines(List<ClientTooltipComponent> components, Font textRenderer, int maxSize) {
		for (int i = 0; i < components.size(); i++) {
			if (components.get(i) instanceof ClientTextTooltip orderedTextTooltipComponent) {
				Component text = OrderedTextToTextVisitor
						.get(((OrderedTextToolTipAccessor) orderedTextTooltipComponent).getText());
				if (text.getSiblings().isEmpty())
					continue;
				
				List<ClientTooltipComponent> wrapped = textRenderer.split(text, maxSize).stream()
		...
		for (int i = 0; i < components.size(); i++) {
			if (components.get(i) instanceof ClientTextTooltip orderedTextTooltipComponent) {
				...
				for (int j = 0; j < children.size() - 1; j++) {
					...
					if (code.equals("\\n")) {
						components.set(i,
								ClientTooltipComponent.create(textWithChildren(children, 0, j).getVisualOrderText()));
						components.add(i + 1, ClientTooltipComponent
								.create(textWithChildren(children, j + 2, children.size()).getVisualOrderText()));
						break;
					}
				}
			}
